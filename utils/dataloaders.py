...
class LoadStreams:
    ...
    def __init__(self, sources='file.streams', img_size=640, stride=32, auto=True, transforms=None, vid_stride=1):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            # - cv2.CAP_ANY (DEFAULT: tries all)
            
            # Define possible backend options
            backends = [
                cv2.CAP_V4L2,          # Video4Linux2 (Linux)
                cv2.CAP_GSTREAMER,     # GStreamer (Linux)
                cv2.CAP_DSHOW,         # DirectShow (Windows)
                cv2.CAP_FFMPEG,        # FFmpeg (Linux, Windows)
                cv2.CAP_IMAGES,        # Image sequence (Windows, Linux)
                cv2.CAP_MSMF,          # Media Foundation (Windows)
                cv2.CAP_AVFOUNDATION,  # AVFoundation (macOS)
            ]

            # Loop through all the backends and try to open the video capture
            for backend in backends:
                print('trying opencv backend:', cv2.videoio_registry.getBackendName(backend))
                cap = cv2.VideoCapture(s, backend)
                if cap.isOpened():
                    print(f"Video capture opened successfully with backend {cv2.videoio_registry.getBackendName(backend)}")
                    break

            # Check if video capture was successfully opened
            if not cap.isOpened():
                print("Failed to open video capture with backends:", backends)
                print("Doing final try with backend cv2.CAP_ANY", cv2.CAP_ANY)
                
                cap = cv2.VideoCapture(s, cv2.CAP_ANY)
                if not cap.isOpened():
                    print("All backends failed, exiting...")
                    # handle error here
                    exit()
            ## added by andrew =================================================================================================
