...
---------------Things to do-----------------
  ...
  9. heatmap?                                                               [     ]
 10. dewarp fish-eye                                                        [done!] reliably?
 11. multiple images, crop together, then feed to detector                  [     ] next!
...
def draw_rectangle(event, x_new, y_new, flags, params):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if w >= 20 and h >= 20:
            ...
            zone = {'id': zone_id, 'color': color, 'roi': (x, y, w, h), 'zone_dict': {'object_count': 0, 'unique_ids': set()}, 'types': []}
...
def draw_tracks(im0, track, zone_roi=None):
    [cv2.line(im0, (int(track.centroidarr[i][0]),int(track.centroidarr[i][1])), 
        (int(track.centroidarr[i+1][0]),int(track.centroidarr[i+1][1])),
        (124, 252, 0), thickness=3) for i,_ in  enumerate(track.centroidarr) 
    ...
    for zone_roi in zone_rois:
        ...
        for bbox, identity, category in zip(bbox_xyxy, identities, categories):
            ...
            if is_inside(bbox, zone_roi['roi']):
                ...
                if track_in_zones_only:
                    # draw tracked object lines on frame
                    draw_tracks(im0, track, zone_roi)

# Initialize a dictionary to track detection confidence statistics for each class
class_conf_stats = {}

def update_conf_stats(class_name, conf):
    if class_name not in class_conf_stats:
        class_conf_stats[class_name] = {'min': conf, 'max': conf, 'sum': conf, 'count': 1, 'avg': conf}
    else:
        stats = class_conf_stats[class_name]
        stats['min'] = min(stats['min'], conf)
        stats['max'] = max(stats['max'], conf)
        stats['sum'] += conf
        stats['count'] += 1
        stats['avg'] = stats['sum'] / stats['count']  # Update average
...
@torch.no_grad()
def detect(video_name,weights=ROOT / 'yolov5n.pt',
    ...
    if load_zones_path is not None:   # Check if a path to a zones file was specified
        with open(load_zones_path, 'r') as f:   # Open the file at the specified path
            ...
            for line in lines:   # Loop through each line in the file
                ...
                zone = {'id': zone_id, 'color': zone_color, 'roi': zone_roi, 'zone_dict': {'object_count': 0, 'unique_ids': set()}, 'types': []}
                zone_rois.append(zone)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if webcam:  # batch_size >= 1
                ...
                p, im0, frame = path, im0s.copy(), getattr(dataset, video_name, 0)
            ...
            if load_zones_path is None and not default_zone_set:
                print('creating default zone')
                
                frame_height, frame_width = im0.shape[:2]
                zone = {'id': 'default', 'color': (0, 255, 0), 'roi': (0, 0, frame_width, frame_height), 'zone_dict': {'object_count': 0, 'unique_ids': set()}, 'types': []}
                print('zone:', zone)
            ...
            if len(det) and len(zone_rois):
                ...
                # Update your detection processing loop
                for x1, y1, x2, y2, conf, detclass in det.cpu().detach().numpy():
                    class_name = category_dict.get(detclass, f"Class {detclass}")  # Convert class ID to name
                    update_conf_stats(class_name, conf)  # Update stats with class name
                    
                    # Proceed with detection processing
                    if not use_conf_thres_per_class or (detclass in conf_thres_per_class and conf >= conf_thres_per_class[detclass]):
                        dets_to_sort = np.vstack((dets_to_sort, np.array([x1, y1, x2, y2, conf, detclass])))
                        avg_conf = class_conf_stats[class_name]['avg']  # Directly access the average confidence
                        print(f"[{source}] Detection {class_name} (confidence: {conf}), avg conf: {avg_conf}")
...
def parse_opt():
    ...
    parser.add_argument('--video_name', type=str, default='stream', help='name of the stream')
