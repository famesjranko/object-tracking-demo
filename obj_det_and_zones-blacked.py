...
#### =======================================================
####  have added drawing to a second image in this function
#### =======================================================
def visualize_tracking_results(im0, tracked_dets, zone_rois, category_dict, track_in_zones_only, track=None):
    ...
    #### =======================================================
    # Check if the mask image exists in the local directory
    ...
    # If mask image doesn't exist, create a blank mask image with the same dimensions as im0
    if mask_image is None:
        mask_image = np.zeros_like(im0)
        cv2.imwrite(mask_image_path, mask_image)  # Save the blank mask image
    #### =======================================================
    ...
    #### =======================================================
    # Display the mask image with circles in a smaller window
    #cv2.namedWindow("Mask Image with Circles", cv2.WINDOW_NORMAL)  # Create a resizable window
    #cv2.imshow("Mask Image with Circles", mask_image)
    #### =======================================================
            ...
            if len(det):
                ...
                for track in tracks:
                    if len(tracked_dets)>0:
                        #### =======================================================
                        #### added return mask_image
                        # draw objects boundary box on frame
                        mask_image = visualize_tracking_results(im0,
                                                   tracked_dets,
                                                   zone_rois,
                                                   category_dict,
                                                   track_in_zones_only,
