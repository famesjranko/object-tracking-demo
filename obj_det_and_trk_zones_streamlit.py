...
def visualize_tracking_results(im0, tracked_dets, zone_rois, category_dict, track_in_zones_only, kpi7_text, track=None):
    ...
    for zone_roi in zone_rois:
        ...
        #zone_name = zone_roi['id']  # Assuming each zone_dict has a name
        #zone_counts[zone_name] = {
        #    'object_count': zone_roi['zone_dict']['object_count'],
        #    'unique_ids': list(zone_roi['zone_dict']['unique_ids'])  # Convert set to list for JSON serialization
        #}
        
    kpi7_text.write(str(zone_count))
    print(zone_count)
...
def detect(weights=ROOT / 'yolov5n.pt',
        ...
        blur_obj=False,color_box = False, stframe=None,
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                # Check if there are detections to sort
                if len(dets_to_sort) > 0:
                    ...
                    if len(tracks) > 0:
                        # Loop over tracks
                        for track in tracks:
                            # Ensure there are tracked detections to process
                            if len(tracked_dets) > 0:
                                # Draw objects boundary box on frame and possibly update kpi7_text
                                zone_count = visualize_tracking_results(im0,
                                                                        tracked_dets,
                                                                        zone_rois,
                                                                        category_dict,
                                                                        track_in_zones_only,
                                                                        kpi7_text,
                                                                        track)
                                # Assuming visualize_tracking_results or some logic here updates kpi7_text
                            else:
                                # This else is redundant if you're setting kpi7_text to "0" by default and there are tracks
                                pass
                    
                                                   
                #r.set('zone_counts', json.dumps(zone_counts))
                #import json
                #data = {'zone_counts': zone_counts}
...
def parse_opt():
    ...
    parser.add_argument('--stframe', type=str, default=None, help='streemlit strframe')
